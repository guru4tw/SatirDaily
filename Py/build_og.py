...
def draw_card(font_path, *, title, when, meta, brand="SatirDaily｜薩提爾活動每日匯整"):
    ...
    # 日期膠囊
    if when:
        f_when = _font(font_path, 38)
        tw = d.textlength(when, font=f_when)
        d.rounded_rectangle([PAD, y + 14, PAD + tw + 56, y + 80],
                            radius=33, fill=(255, 255, 255))
        d.text((PAD + 28, y + 22), when, font=f_when, fill=BG_BOT)
        y += 104

    # 次要資訊(地區/講師/主辦)
    if meta:
        f_meta = _font(font_path, 36)
        meta_line = _wrap(d, meta, f_meta, W - 2 * PAD, 1)[0]
        d.text((PAD, y + 18), meta_line, font=f_meta, fill=SUB)
