...
> 自動產生於 **2026-06-23 14:12**，由 `discover_sources.py` 讀 `Satir_website.md` 產出。
> **本檔為自動產物，請勿手改**；要增刪來源請改 `Satir_website.md`，再重跑腳本。
>
...
- **連線檢查**：🟢 可連線 (HTTP 200)　（檢查時間 2026-06-23 14:12）
...
1. 在 `Satir_website.md` 加一行：`- [名稱](入口URL)`（其後可接 ` | 地區 | 備註`，只有 URL 必填）。
