...
> 自動產生於 **2026-07-19 21:58**，由 `discover_sources.py` 讀 `/home/runner/work/SatirDaily/SatirDaily/Docs/Satir_website.md` 產出。
...
| `satir_org` | 台灣薩提爾成長模式推展協會 | — | ⬜ 待勘查 | 🔴 連線失敗 (ReadTimeout) | [列表頁](https://www.satir.org.tw/) |
...
- **連線檢查**：🟢 可連線 (HTTP 200)　（檢查時間 2026-07-19 21:57）
...
- **連線檢查**：🔴 連線失敗 (ReadTimeout)　（檢查時間 2026-07-19 21:57）
...
- **連線檢查**：🟢 可連線 (HTTP 200)　（檢查時間 2026-07-19 21:58）
- **Content-Type**：`text/html`
- **頁面標題**：薩提爾 - 心流逸境教育平台
- **結構線索（自動偵測，需人工確認）**：疑似 WordPress（可試 /wp-json REST API）；含 JSON-LD 結構化資料（可直接取 Event schema）
- **建議解析路徑**：
