...
- **首頁語錄精簡** — 四位(維琴尼亞·薩提爾 / 瑪莉亞·葛茉莉 / 約翰·貝曼 / 成蒂)各保留一句代表語錄,刪除其餘兩句。語錄為 `index.html` voices 區靜態內容,`build_site_json.py` 只注入 JSON-LD 與清單標記區、不動此段,直接改檔即可。
- **活動子頁顯示摘要圖** — og:image 原本只是分享 meta(LINE/FB 預覽用),頁面本身看不到。`build_site_json.py` 的 `event_page()` 在標題上方加一張 `<img class="hero-og">` 指向 `assets/og/{id}.png`,讓使用者進子頁就看到 1200×630 摘要卡;og 不存在則不輸出(沿用 `og_url_if_exists` graceful)。只加在活動子頁,講師/地區彙整頁不放。補 `.hero-og` CSS。
- **首頁 header 微調** — (1) 兩段副標(`.voices-intro`「薩提爾模式的傳承…」、活動區 `.lede`「陪你找到…」)由 `--ink-soft` 提到 `--ink`、weight 加到 500,改善低對比(含瀏覽器強制深色反轉下的可讀性)。(2) 「上次更新 日期」eyebrow 從 hero 上方移到「共 N 場活動」徽章右側(新增 `.updated-row` flex 容器),`#lastUpd`/`#updated` id 不變、JS 不動。(3) 移除 logo 旁「每日匯整」tag。
